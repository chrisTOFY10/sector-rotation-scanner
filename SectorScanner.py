...
# --- PAGE SETUP ---
st.set_page_config(page_title="Industry Rotation Scanner", layout="wide")

# --- SIDEBAR NAVIGATION ---
st.sidebar.title("Navigation")
page = st.sidebar.radio("Select a Page:", ["Scanner Dashboard", "Industry Heatmap 📊", "How It Works (Metrics)"])
st.sidebar.divider()
...
if page == "Scanner Dashboard":
    st.title("🔄 Industry Rotation & Relative Strength Scanner")
    st.markdown("Identify capital flows and find the strongest stocks within leading industries.")
    ...
    with st.spinner("Fetching latest market data from Google Sheets..."):
        try:
            ...
            # Clean Exchange and Industry columns
            if 'Exchange' in df.columns:
                df['Exchange'] = df['Exchange'].astype(str).str.strip()
            if 'Industry' in df.columns:
                df['Industry'] = df['Industry'].astype(str).str.strip()
            else:
                st.error("⚠️ The column 'Industry' was not found in your data. Please check your data source.")
                st.stop()
        ...
        # --- PHASE 1: INDUSTRY ROLL-UP ---
        st.header("Phase 1: Industry Rotation (Capital Flows)")
        
        industry_df = df.groupby(['Date', 'Industry']).agg({'Close': 'mean', 'Volume': 'sum'}).reset_index()
        industry_metrics = []
        industries = industry_df['Industry'].unique()
        
        for industry in industries:
            i_data = industry_df[industry_df['Industry'] == industry].sort_values('Date').copy()
            i_data['Short_Return'] = i_data['Close'].pct_change(periods=short_window) * 100
            i_data['Long_Return'] = i_data['Close'].pct_change(periods=long_window) * 100
            i_data['Vol_Baseline'] = i_data['Volume'].rolling(window=vol_window).mean()
            
            latest_data = i_data.iloc[-1]
        ...
        industry_results = pd.DataFrame(industry_metrics).dropna().sort_values(by='Momentum Acceleration', ascending=False)
        st.dataframe(industry_results, use_container_width=True)
        st.divider()

        # --- PHASE 2: INDIVIDUAL STOCK SCREENER ---
        st.header("Phase 2: Stock Screener (Relative Strength & Breakouts)")
        selected_industry = st.selectbox("Select a Leading Industry to Scan:", options=industry_results['Industry'].tolist())
        
        if selected_industry:
            stock_df = df[df['Industry'] == selected_industry].copy()
            stock_metrics = []
            tickers = stock_df['Symbol'].unique()
            min_required = max(long_window + 1, vol_window, rs_window)
            
            with st.spinner(f"Scanning stocks in {selected_industry}..."):
                for ticker in tickers:
                    t_data = stock_df[stock_df['Symbol'] == ticker].sort_values('Date').copy()
                    if len(t_data) < min_required: continue 
                    
                    t_data = pd.merge(t_data, industry_df[industry_df['Industry'] == selected_industry][['Date', 'Close']], on='Date', suffixes=('', '_Industry'))
                    t_data['RS'] = t_data['Close'] / t_data['Close_Industry']
            ...
            if stock_metrics:
                ...
                strong_stocks = final_stocks[(final_stocks['RS vs Industry'] == 'Uptrend')].sort_values(by=f'Daily Vol vs {vol_window}D Avg', ascending=False)
                
                cols = ['Symbol', 'Exchange', 'Latest Close', 'RS vs Industry', f'Daily Vol vs {vol_window}D Avg', 'Volume Breakout (>150%)']
                ...
                st.warning(f"No stocks in {selected_industry} have enough historical data.")


# ==========================================
# PAGE 2: INDUSTRY HEATMAP
# ==========================================
elif page == "Industry Heatmap 📊":
    st.title("📊 Market Heatmap")
    st.markdown("Visualize capital flow and performance across all industries simultaneously. Block size represents **Volume**, while color represents **Percentage Return**.")
    ...
    with st.spinner("Generating heatmap..."):
        try:
            ...
            if 'Industry' not in df.columns:
                st.error("⚠️ The column 'Industry' was not found in your data.")
                st.stop()
    ...
    st.sidebar.caption("💡 **Tip:** Lower this number to make the big Industry blocks pop. Raise it to highlight extreme individual stock moves.")
    ...
    latest_df['Industry'] = latest_df['Industry'].astype(str).replace(['nan', 'None', ''], 'Unclassified')
    ...
    if not latest_df.empty:
        fig = px.treemap(
            latest_df,
            path=[px.Constant("Overall Market"), 'Industry', 'Symbol'],
            values='Volume',
            color='Return (%)',
...
# ==========================================
    ...
    st.markdown("**The Concept:** This measures whether an industry's trend is speeding up or running out of gas. An industry might be up overall, but if its recent gains are shrinking, the trend is dying.")
    ...
    st.success("**💡 Real-World Insight:** If the Gold Mining industry is up 5% over the long-term, but up 8% in the short-term, its acceleration is **+3.00**. This tells you new buyers are aggressively stepping in *right now*. You want to look for positive numbers at the top of the Phase 1 chart.")
    st.write("---")

    st.header("2. Volume Surge (Industry Level)")
    st.markdown("**The Concept:** Price moves can be faked by a lack of liquidity, but massive trading volume cannot be faked. This metric measures the influx of broad capital into an entire industry relative to its normal baseline.")
    st.markdown("**👣 The Novice Analogy:** Think of volume like footprints in the snow. A few retail traders leave a light trail. But when hedge funds and mutual funds buy into an industry, they march an entire army through the snow. This metric looks for the army.")
    st.latex(r"\text{Industry Surge} = \frac{V_{current}}{\overline{V}_{baseline}}")
    st.success("**💡 Real-World Insight:** A ratio of **1.50** means today's industry volume is 50% higher than its recent average. If an industry is accelerating in price *and* has a volume ratio above 1.0, the move is legitimate. Institutions are actively buying.")
    st.write("---")

    st.header("3. RS vs Industry (Relative Strength)")
    st.markdown("**The Concept:** This evaluates a stock's performance directly against its own industry peers rather than the broad market.")
    st.markdown("**🏃‍♂️ The Novice Analogy:** Imagine a track team running a race with a heavy tailwind. The whole team will run faster times (the industry is going up). But Relative Strength asks: *Who is the fastest runner on the team?* You don't just want a stock that is being dragged up by the wind; you want the strongest athlete.")
    st.latex(r"RS = \frac{P_{stock}}{P_{industry}}")
    st.success("**💡 Real-World Insight:** If the scanner shows **'Uptrend'**, it means this specific stock is gaining more on green days and losing less on red days compared to its industry peers. This is your prime target.")
    st.write("---")
